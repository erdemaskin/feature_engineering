...
# Problem: It is desired to develop a machine learning model that can predict whether people have diabetes when their characteristics are specified. You are expected to perform the necessary data analysis and feature engineering steps before developing the model.
...
# Data used for diabetes research on Pima Indian women aged 21 and over living in Phoenix, the 5th largest city in the State of Arizona in the USA. It consists of 768 observations and 8 numerical independent variables.
...
for col in num_cols:
    target_summary_with_num(df[df["SkinThickness"] > 0], "Outcome", col)
...
df[df["Insulin"] > 0].corr()
df[df["SkinThickness"] > 0].corr()
# Korelasyon Matrisi
...
# Filling in Missing Values We do not know the target of the new patient. Can also be filled with knn
for col in zero_columns:
...
df["NEW_AGE_BMI_NOM"].value_counts()
...
df["NEW_AGE_GLUCOSE_NOM"].value_counts()
...
df["INSULIN"].median()
dff = df[df["INSULIN"] != -0.1827488580040976]
dff.shape
...
(df_yedek["Age"] - 18)*12 / df_yedek["Pregnancies"]
